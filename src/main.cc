...
uint64_t warmup_instructions = 1000000,
         ...
         champsim_seed;
uint32_t measure_dram_bw_epoch = 256;
...
#ifdef MEASURE
PerformanceCounter total_perf_counter[NUM_CPUS];
#endif
...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
  ...
  cout << "  LATE: " << setw(10) << cache->pf_late << endl;
  ...
  cout << cache->NAME;
  cout << " AVERAGE LOAD MISS LATENCY: " << double(cache->miss_latency[cpu][0]) / cache->roi_miss[cpu][0] << " cycles" << endl;

  cout << cache->NAME;
  cout << " AVERAGE RFO MISS LATENCY: " << double(cache->miss_latency[cpu][1]) / cache->roi_miss[cpu][1] << " cycles" << endl;

  cout << cache->NAME;
  cout << " AVERAGE PREFETCH MISS LATENCY: " << double(cache->miss_latency[cpu][2]) / cache->roi_miss[cpu][2] << " cycles" << endl;

  cout << cache->NAME;
  cout << " AVERAGE WRITEBACK MISS LATENCY: " << double(cache->miss_latency[cpu][3]) / cache->roi_miss[cpu][3] << " cycles" << endl;

#ifdef MEASURE_COMPULSORY
  cout << cache->NAME << " " << cache->cmr_.to_string() << endl;
#endif
}
...
int main(int argc, char **argv)
{
  ...
  cout << "**************************" << endl;
  cout << "Prefetch will query the cache and affect the replacement policy!" << endl;
  cout << "**************************" << endl;
#endif
  // initialize knobs
  ...
  while (1)
  {
    ...
    c = getopt_long_only(argc, argv, "wihscbt", long_options, &option_index);
  ...
  // zeal4u:be compatible with the results obtained under the original environment.
  DRAM_DBUS_RETURN_TIME = DRAM_MTPS > CPU_FREQ ? (BLOCK_SIZE / DRAM_CHANNEL_WIDTH) * (1.0 * CPU_FREQ / DRAM_MTPS) : (BLOCK_SIZE / DRAM_CHANNEL_WIDTH) * (CPU_FREQ / DRAM_MTPS);
...
#ifdef MEASURE
        cout << "Performance Counter for CPU " << i << " Begin" << endl;
        batch_perf_counter[i].output();
        total_perf_counter[i] = total_perf_counter[i] + batch_perf_counter[i];
        batch_perf_counter[i].reset();
        cout << "Performance Counter for CPU " << i << " End" << endl;
#endif
      }
...
#ifdef MEASURE
        cout << "Total Performance Counter for CPU " << i << " Begin" << endl;
        total_perf_counter[i] = total_perf_counter[i] + batch_perf_counter[i];
        total_perf_counter[i].output();
        cout << "Total Performance Counter for CPU " << i << " End" << endl;
#endif
      }
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++)
  {
    ...
    cout << "L1D WB Stall Cycle: " << ooo_cpu[i].L1D.STALL[RFO] << endl;
  }
